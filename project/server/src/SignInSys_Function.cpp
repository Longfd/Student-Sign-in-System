...
int insertActSqlOpt(int conn_no, ActivityReq& actReq, std::string& err)
{
	...
	write_debug_log("In Func[%s] : 查询学生信息成功! 学生数:%d", funcName, iCount);

	// 2.遍历学生信息 并 插入到签到表
	std::ostringstream ossInert;
	for (int i = 0; i < iCount; ++i){
		ossInert << "INSERT INTO SINGIN_TBL(act_no,act_name, s_id,s_name, cls_no, cls_name, sign_status) VALUES("
			<< actReq.actNo << ",'"
			<< actReq.actName << "','"
			<< row[0] << "','"//s_id学生号
			<< row[1] << "',"//学生名
	...
	mysql_free_result(result);
	write_debug_log("In Func[%s] : SQL:\n%s", funcName, ossInert.str().c_str());
...
int addActivity(CommThreadInfo* thread_info, unsigned char* data)
{
	...
	try{
		...
		if (0 != iRet){
			...
			jret["msg"] = str_err.c_str();
			str_ret = jret.dump();
			write_debug_log("In Func[%s] : insertActivity() fail ! iRet:%d, str_err:%s", funcName, iRet, str_err.c_str());
		}
		else{
			jret["result"] = "0";
			jret["msg"] = "success";
